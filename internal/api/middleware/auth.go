...
//
// When cfg.AuthEnabled is false the middleware is a no-op, making it safe
// to use in local development outside a cluster.
func NewAuthMiddleware(cfg *config.Config) gin.HandlerFunc {
	...
	if err != nil {
		panic("fusion-content: auth enabled but cannot build in-cluster K8s client: " + err.Error())
	}
...
//
// The pod's own SA token is re-read on every call because the kubelet rotates
// projected tokens periodically — caching it would cause auth failures after rotation.
